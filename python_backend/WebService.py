    ...
    def get(self, *args):
        ...
        try:
            ...
            if(exchange == "Both"):
               ...
               condition = "AND exchange = '" + exchange + "'"
        ...
        try:
            ...
            if (pairname == "Both"):
                ...
                condition = condition +" "+ "AND pairname = '" + pairname + "'"
        ...
        print("SELECT * FROM orderBooks WHERE type = 'ask' AND price > "+price +" " + condition +" ORDER BY id DESC LIMIT 30 ")
        # SELECT * FROM table WHERE column LIKE "%" use wildcard at client side when seleting every values

        rows_ask = conn.execute("SELECT * FROM orderBooks WHERE type = 'ask' AND price > "+price +" " + condition +" ORDER BY id DESC LIMIT 15 ")
        rows_bid = conn.execute("SELECT * FROM orderBooks WHERE type = 'bid' AND price > "+price + " "+condition + " ORDER BY id DESC LIMIT 15 ")
